...
int A0_state; assign A0_state to "{P}A0_state"; monitor A0_state;
int OMTX_state; assign OMTX_state to "{P}OMTX_state"; monitor OMTX_state;
...
ss orient {
 ...
 state init {
  when (startup != 12345) {
   ...
   if (orient_pmac_PM == 0) {
    {strncpy(Msg, ("motor prefix (PM) not specified"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
   } else {
    ...
    if (orient_pmac_mTTH == 0) {
     {strncpy(Msg, ("orient: TTH motor name (mTTH) not specified"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
    } else {
    ...
    if (orient_pmac_mTH == 0) {
     {strncpy(Msg, ("orient: TH motor name (mTH) not specified"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
    } else {
    ...
    if (orient_pmac_mCHI == 0) {
     {strncpy(Msg, ("orient: CHI motor name (mCHI) not specified"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
    } else {
    ...
    if (orient_pmac_mPHI == 0) {
     {strncpy(Msg, ("orient: PHI motor name (mPHI) not specified"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
    } else {
  ...
  when (A0_state == 0) {
   ...
   if (lambda > 1.0e-9) {
    ...
    {(load_energy) = (energy); pvPut(load_energy);};
    if (orientDebug) {strncpy(Msg, ("calculated energy"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   ...
   if ((a!=0) && (b!=0) && (c!=0) && (alpha!=0) && (beta!=0) && (Gamma!=0) && (lambda!=0)) {
    ...
    if (orientDebug) {strncpy(Msg, ("calling calc_A0"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    ...
    if (A0_state == 1) {strncpy(Msg, ("Successful A0 calc"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   } else {
...
# 523 "../orient_st.st"
   if ((H1!=0 || K1!=0 || L1!=0) && (H2!=0 || K2!=0 || L2!=0)) {
    if (orientDebug) {strncpy(Msg, ("calling calc_OMTX"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   ...
   if ((i==0) && (fabs(errAngle) < errAngleThresh)) {
    ...
    {strncpy(Msg, ("Successful OMTX calc"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   } else {
    {(OMTX_state) = (2); pvPut(OMTX_state);};
    if (motPut_Auto != 0) {
     {(motPut_Auto) = (0); pvPut(motPut_Auto);};
     {strncpy(Msg, ("Bad OMTX calc; motPut_Auto set to Manual"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
    }
   }
  } state init
...
# 586 "../orient_st.st"
 ...
 state enter_monitor_all {
  when (delay(10)) {
   ...
   efClear(Mode_mon);
   {strncpy(Msg, ("Initialization complete; going live"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
  } state monitor_all
 }
 ...
 state monitor_all {
  ...
  when (motorsConnected == 0) {
   {strncpy(Msg, ("Waiting for motor connection(s)"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
  } state monitor_all_delay

  when (motorStopsConnected == 0) {
   {strncpy(Msg, ("Waiting for motor-stop connection(s)"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
  } state monitor_all_delay


  when ((A0_state == 1) && OMTX_Calc) {

   if (orientDebug) {strncpy(Msg, ("Recalculating OMTX"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   %%pVar->v1_hkl[H_INDEX] = pVar->H1; pVar->v1_hkl[K_INDEX] = pVar->K1; pVar->v1_hkl[L_INDEX] = pVar->L1;
   %%pVar->v1_angles[TTH_INDEX] = pVar->TTH1; pVar->v1_angles[TH_INDEX] = pVar->TH1;
   %%pVar->v1_angles[CHI_INDEX] = pVar->CHI1; pVar->v1_angles[PHI_INDEX] = pVar->PHI1;
   %%pVar->v2_hkl[H_INDEX] = pVar->H2; pVar->v2_hkl[K_INDEX] = pVar->K2; pVar->v2_hkl[L_INDEX] = pVar->L2;
  ...
  when (efTest(xtalParm_mon) || efTest(lambda_mon) || efTest(energy_mon)) {
   ...
   efClear(xtalParm_mon); efClear(lambda_mon); efClear(energy_mon);
   if (orientDebug) {strncpy(Msg, ("new XTAL parameters"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   ...
   {(A0_state) = ((i==0 ? 1 : 2)); pvPut(A0_state);};
   if (A0_state == 1) {
    {strncpy(Msg, ("Successful A0 calc"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   } else {
    {strncpy(Msg, ("A0 calc failed"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
   }
  } state newHKL
  ...
  when (efTest(A0_mon)) {
   efClear(A0_mon);
   if (orientDebug) {strncpy(Msg, ("new A0 precursor(s)"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   ...
   if (i == -1) {
    {strncpy(Msg, ("Could not invert A0 array"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    {(A0_state) = (2); pvPut(A0_state);};
   }
  } state newA0
  ...
  when (efTest(OMTX_mon)) {
   efClear(OMTX_mon);
   if (orientDebug) {strncpy(Msg, ("new OMTX element(s)"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   ...
   if (i == -1) {
    {strncpy(Msg, ("Could not invert OMTX array"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    {(OMTX_state) = (2); pvPut(OMTX_state);};
   } else if (A0_state != 1) {
    {strncpy(Msg, ("Can't check or use OMTX array (No A0)"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    {(OMTX_state) = (2); pvPut(OMTX_state);};
   } else {
    errAngle = checkOMTX(v2_hkl, v2_angles, A0, A0_i, OMTX_i);
    pvPut(errAngle);
    if (fabs(errAngle) < errAngleThresh) {
     ...
     {(OMTX_state) = (1); pvPut(OMTX_state);};
     if (orientDebug) {strncpy(Msg, ("User's OMTX passes test"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    } else {
     {(OMTX_state) = (2); pvPut(OMTX_state);};
     if (orientDebug) {strncpy(Msg, ("User's OMTX fails test"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    }
   }
  } state newOMTX
...
# 800 "../orient_st.st"
  ...
  when (efTest(Mode_mon)) {
   efClear(Mode_mon);
   if (orientDebug) {
    {strncpy(Msg, ("new HKL-to-Angles Mode"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
    printf("...Mode=%d\n", Mode);
   }
  } state newHKL
  ...
  when (opAck) {
   {strncpy(Msg, (""), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
   {(opAck) = (0); pvPut(opAck);};
   {(Alert) = (0); pvPut(Alert);};
  } state monitor_all
 }
...
# 969 "../orient_st.st"
 state newHKL {
  when ((A0_state != 1) || (OMTX_state != 1)) {
   if (A0_state != 1) {
    {strncpy(Msg, ("We don't have a good A0 matrix"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
   } else {
    {strncpy(Msg, ("We don't have a good OMTX matrix"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
   }
  } state monitor_all
  when (delay(0.02)) {
   ...
   if (i == 0) {
    ...
    {strncpy(Msg, ("Bad HKL2angles calc."), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
   }
  } state putAll
 }
  ...
  when () {
   ...
   if (newMotors) {
    ...
    if (motTTH_Connected && motTH_Connected && motCHI_Connected && motPHI_Connected) {
     ...
     {strncpy(Msg, ("Motors not connected"), 39); pvPut(Msg); {(Alert) = (1); pvPut(Alert);}; printf("orient: %s\n", Msg);};
     {(newMotors) = (0); pvPut(newMotors);};
    }
   }
...
ss stop {
 state stop_all {
  when (stopMotors) {
   {strncpy(Msg, ("stopping motors"), 39); pvPut(Msg); if (orientDebug) printf("orient: %s\n", Msg);};
