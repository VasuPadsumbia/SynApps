...
unsigned int raw_a; assign raw_a to "{P}current:a:raw" ;
unsigned int raw_b; assign raw_b to "{P}current:b:raw" ;
unsigned int raw_c; assign raw_c to "{P}current:c:raw" ;
unsigned int raw_d; assign raw_d to "{P}current:d:raw" ;

int current_low_raw; assign current_low_raw to "{P}current:low:raw" ; monitor current_low_raw;
...
int offset [ 24 ]; assign offset to { "{P}r1:A2", "{P}r1:B2", "{P}r1:C2", "{P}r1:D2", "{P}r2:A2", "{P}r2:B2", "{P}r2:C2", "{P}r2:D2", "{P}r3:A2", "{P}r3:B2", "{P}r3:C2", "{P}r3:D2", "{P}r4:A2", "{P}r4:B2", "{P}r4:C2", "{P}r4:D2", "{P}r5:A2", "{P}r5:B2", "{P}r5:C2", "{P}r5:D2", "{P}r6:A2", "{P}r6:B2", "{P}r6:C2", "{P}r6:D2" } ; monitor offset;
...
ss sncqxbpm {
  ...
  state init {
    when (!enable) {
      if (debug_flag >= 0) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 328, SNLtaskName, 0);; printf("%s\n", "software disabled"); epicsThreadSleep(0.01); };
    } state disable
    when (init) {
...
# 364 "../sncqxbpm.st"
  ...
  state disable {
    when (enable) {
      { init = ( 1 ); pvPut(init,SYNC); };
      if (debug_flag >= 0) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 386, SNLtaskName, 0);; printf("%s\n", "software re-enabled"); epicsThreadSleep(0.01); };
    } state idle
  }
  ...
  state comm_error {
    when (init || !s_sevr || delay((0.5 * 60.0))) {
      ...
      if (debug_flag >= 20) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 396, SNLtaskName, 20);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
      s_sevr = 0;
      { init = ( 1 ); pvPut(init,SYNC); };
    } state init
  }
  ...
  state idle {
    ...
    when (init) {
      if (debug_flag >= 20) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 415, SNLtaskName, 20);; printf("%s\n", "user re-initialized"); epicsThreadSleep(0.01); };
    } state init


    when (efTestAndClear(gain_mon)) {
      sprintf (debug_str, "gain changed to %s (%d)", gainStr[gain],gain);
      if (debug_flag >= 5) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 421, SNLtaskName, 5);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
      { sprintf(bpm_command, ":CONF%d:CURR:RANG %d", bpm_address, gain+1, 0, 0); sprintf(debug_str, "BPM_COMM: <%s> (%d) <-- <%s>", s_aout, 1, bpm_command); if (debug_flag >= 15) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 422, SNLtaskName, 15);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; if (s_tmod != 1) { s_tmod = ( 1 ); pvPut(s_tmod,SYNC); }; {epicsThreadSleep(1/60.);}; if (1 == 1) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } } else { strcpy (bpm_response, ""); for (retry = 0; retry < 3; retry++) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } for (i = 0; i < 0.25; i++) { {epicsThreadSleep(1/60.);}; if (efTest(s_ainp_mon)) break; } if (!s_sevr && efTest(s_ainp_mon)) { sprintf(debug_str, "BPM_COMM: <%s> ==> <%s>", s_aout,s_ainp); if (debug_flag >= 10) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 422, SNLtaskName, 10);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; strcpy (bpm_response, s_ainp+1); break; } } } };
    ...
    when (efTestAndClear(signal_mode_mon) || efTestAndClear(buflen_mon)) {
      ...
      if (signal_mode == 0) {
        ...
        if (debug_flag >= 5) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 441, SNLtaskName, 5);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
        { sprintf(bpm_command, ":CONF%d:SINGLE", bpm_address, 0, 0, 0); sprintf(debug_str, "BPM_COMM: <%s> (%d) <-- <%s>", s_aout, 1, bpm_command); if (debug_flag >= 15) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 442, SNLtaskName, 15);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; if (s_tmod != 1) { s_tmod = ( 1 ); pvPut(s_tmod,SYNC); }; {epicsThreadSleep(1/60.);}; if (1 == 1) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } } else { strcpy (bpm_response, ""); for (retry = 0; retry < 3; retry++) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } for (i = 0; i < 0.25; i++) { {epicsThreadSleep(1/60.);}; if (efTest(s_ainp_mon)) break; } if (!s_sevr && efTest(s_ainp_mon)) { sprintf(debug_str, "BPM_COMM: <%s> ==> <%s>", s_aout,s_ainp); if (debug_flag >= 10) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 442, SNLtaskName, 10);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; strcpy (bpm_response, s_ainp+1); break; } } } };
      }
      if (signal_mode == 1) {
        sprintf (debug_str, "gain changed to %s (%d) with buffer length = %d",
          signal_modeStr[signal_mode], signal_mode, buflen);
        if (debug_flag >= 5) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 447, SNLtaskName, 5);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
        { sprintf(bpm_command, ":CONF%d:AVGCURR %d", bpm_address, buflen, 0, 0); sprintf(debug_str, "BPM_COMM: <%s> (%d) <-- <%s>", s_aout, 1, bpm_command); if (debug_flag >= 15) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 448, SNLtaskName, 15);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; if (s_tmod != 1) { s_tmod = ( 1 ); pvPut(s_tmod,SYNC); }; {epicsThreadSleep(1/60.);}; if (1 == 1) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } } else { strcpy (bpm_response, ""); for (retry = 0; retry < 3; retry++) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } for (i = 0; i < 0.25; i++) { {epicsThreadSleep(1/60.);}; if (efTest(s_ainp_mon)) break; } if (!s_sevr && efTest(s_ainp_mon)) { sprintf(debug_str, "BPM_COMM: <%s> ==> <%s>", s_aout,s_ainp); if (debug_flag >= 10) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 448, SNLtaskName, 10);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; strcpy (bpm_response, s_ainp+1); break; } } } };
      }
      if (signal_mode == 2) {
        sprintf (debug_str, "gain changed to %s (%d) with buffer length = %d",
          signal_modeStr[signal_mode], signal_mode, buflen);
        if (debug_flag >= 5) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 453, SNLtaskName, 5);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
        { sprintf(bpm_command, ":CONF%d:WDWCURR %d", bpm_address, buflen, 0, 0); sprintf(debug_str, "BPM_COMM: <%s> (%d) <-- <%s>", s_aout, 1, bpm_command); if (debug_flag >= 15) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 454, SNLtaskName, 15);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; if (s_tmod != 1) { s_tmod = ( 1 ); pvPut(s_tmod,SYNC); }; {epicsThreadSleep(1/60.);}; if (1 == 1) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } } else { strcpy (bpm_response, ""); for (retry = 0; retry < 3; retry++) { if (strcmp(s_aout, bpm_command)) { { strcpy(s_aout,bpm_command); pvPut(s_aout,SYNC); epicsThreadSleep(0.01); }; {epicsThreadSleep(1/60.);}; } else { { s_proc = ( 1 ); pvPut(s_proc,SYNC); }; {epicsThreadSleep(1/60.);}; } for (i = 0; i < 0.25; i++) { {epicsThreadSleep(1/60.);}; if (efTest(s_ainp_mon)) break; } if (!s_sevr && efTest(s_ainp_mon)) { sprintf(debug_str, "BPM_COMM: <%s> ==> <%s>", s_aout,s_ainp); if (debug_flag >= 10) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 454, SNLtaskName, 10);; printf("%s\n", debug_str); epicsThreadSleep(0.01); }; strcpy (bpm_response, s_ainp+1); break; } } } };
      }
    } state idle
 ...
 pvPut (raw_d);
  ...
  if (debug_flag >= 12) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 478, SNLtaskName, 12);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
        sprintf (debug_str, "raw: a=%d  b=%d  c=%d  d=%d",
          raw_a, raw_b, raw_c, raw_d);
  if (debug_flag >= 12) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 481, SNLtaskName, 12);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
        { current_a = ( gainTrim[((gain)*4 + (1) - 1)]*( raw_a - offset[((gain)*4 + (1) - 1)] ) ); pvPut(current_a,SYNC); };
        { current_b = ( gainTrim[((gain)*4 + (2) - 1)]*( raw_b - offset[((gain)*4 + (2) - 1)] ) ); pvPut(current_b,SYNC); };
        { current_c = ( gainTrim[((gain)*4 + (3) - 1)]*( raw_c - offset[((gain)*4 + (3) - 1)] ) ); pvPut(current_c,SYNC); };
        { current_d = ( gainTrim[((gain)*4 + (4) - 1)]*( raw_d - offset[((gain)*4 + (4) - 1)] ) ); pvPut(current_d,SYNC); };
...
ss sncqxbpm_tools {
 ...
 epicsThreadSleep(settling);
  ...
  if (debug_flag >= 18) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 540, SNLtaskName, 18);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
 ...
 for (channel=1; channel <= 4; channel++) {
    ...
    if (debug_flag >= 19) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 549, SNLtaskName, 19);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
 }
      }
      ...
      if (debug_flag >= 18) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 556, SNLtaskName, 18);; printf("%s\n", debug_str); epicsThreadSleep(0.01); };
    } state idle

    when (set_defaults) {
      if (debug_flag >= 18) { printf("<%s,%d,%s,%d> ", "../sncqxbpm.st", 560, SNLtaskName, 18);; printf("%s\n", "restore default calibrations"); epicsThreadSleep(0.01); };
